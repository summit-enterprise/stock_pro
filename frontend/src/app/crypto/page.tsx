...
export default function CryptoPage() {
  ...
  const fetchCryptoData = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/api/crypto/market?limit=250', {
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          if (result.data.length > 0 && !selectedCrypto) {
              ...
              <tbody>
                {paginatedData.map((crypto) => (
                  ...
                  >
                    ...
                    <td className={`py-3 px-4 text-sm text-right font-medium ${getChangeColor(crypto.priceChange1h)}`}>
                      {formatPercent(crypto.priceChange1h)}
                    </td>
                    <td className={`py-3 px-4 text-sm text-right font-medium ${getChangeColor(crypto.priceChange24h)}`}>
                      {formatPercent(crypto.priceChange24h)}
                    </td>
                    <td className={`py-3 px-4 text-sm text-right font-medium ${getChangeColor(crypto.priceChange7d)}`}>
          ...
          {filteredAndSortedData.length > itemsPerPage && (
            <div className="mt-6 flex items-center justify-between">
              <div className="text-sm text-gray-700 dark:text-gray-300">
                Showing {startIndex + 1} to {Math.min(endIndex, filteredAndSortedData.length)} of {filteredAndSortedData.length} cryptocurrencies
              </div>
