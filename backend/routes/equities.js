/**
 * Stocks Market Data Routes
 * Provides stock market data (excludes ETFs)
 */
...
/**
 * Get stocks market data (excludes ETFs)
 * GET /api/stocks/market?limit=250
 */
router.get('/market', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 250;
    const cacheKey = `stocks:market:all:${limit}`;
    ...
    if (redisClient && redisClient.isOpen) {
      try {
        ...
        console.warn('Stocks market data: Redis cache read failed');
      }
    }
    ...
    // For stocks, we'll use previous trading days since intraday data may not be available
    ...
    // Fetch equities market data with price changes
    const equitiesResult = await pool.query(
      ...
      WHERE sd.ticker IS NOT NULL
        AND sd.active = true
        AND (sd.type NOT IN ('ETF', 'ETP') OR sd.type IS NULL)
    ...
    // Transform data
    const marketData = equitiesResult.rows.map(row => ({
    ...
    if (redisClient && redisClient.isOpen) {
      try {
        ...
        console.warn('Failed to cache stocks market data in Redis');
      }
    }
